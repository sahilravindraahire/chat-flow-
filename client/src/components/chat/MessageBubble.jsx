...
import {deleteMessage} from "../../features/message/messageSlice.js"
    ...
    <div className={`flex gap-2 group ${isMine ? "flex-row-reverse" : "flex-row"}`}>
      ...
      <div className={`max-w-[65%] ${isMine ? "items-end" : "items-start"} flex flex-col gap-1`}>
        {msg.mediaUrl && msg.messageType === "image" && (
          <img
            src={msg.mediaUrl}
            className="rounded-xl max-w-full max-h-64 object-cover border border-zinc-800"
            alt="media"
          />
        )}
        {msg.mediaUrl && msg.messageType === "video" && (
          <video
            src={msg.mediaUrl}
            controls
            className="rounded-xl max-w-full max-h-64 border border-zinc-800"
          />
        )}
